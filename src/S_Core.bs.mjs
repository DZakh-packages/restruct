...
function recursive(fn) {
  ...
  schema.b = (b, input, selfSchema, path) => {
    ...
    return withPathPrepend(b, input, path, undefined, (b, input, param) => transform(b, input, (_b, input) => {
  ...
  schema["~r"] = () => {
    ...
    let reversed = makeReverseSchema(initialReversed.n, initialReversed.t, initialReversed.m, (b, input, selfSchema, path) => {
      ...
      let opBodyCode = allocateScope(bb) + ("return " + opOutput.i);
      b.c = b.c + ("let " + r + "=" + inputVar + "=>{" + opBodyCode + "};");
      return withPathPrepend(b, input, path, undefined, (_b, input, param) => map(r, input));
    }, initialReversed.f);
    reversed["~r"] = () => schema;
    schema["~r"] = () => reversed;
    return reversed;
...
function factory$2(schema) {
  return makeSchema(() => "array<" + schema.n() + ">", {
    TAG: "array",
    _0: schema
    ...
    let itemOutput = withPathPrepend(bb, itemInput, path, iteratorVar, (b, input, path) => parseWithTypeValidation(b, schema, input, path));
    let itemCode = allocateScope(bb);
...
function typeFilter$1(b, inputVar) {
  let code = "!" + inputVar + "||" + inputVar + ".constructor!==Object";
  let tagged = this.t;
...
function factory$3(schema) {
  return makeSchema(() => "dict<" + schema.n() + ">", {
    ...
    let itemOutput = withPathPrepend(bb, itemInput, path, keyVar, (b, input, path) => parseWithTypeValidation(b, schema, input, path));
    let itemCode = allocateScope(bb);
    let isTransformed = itemInput !== itemOutput;
    let output = isTransformed ? ({
        b: b,
...
function $$catch(schema, getFallbackValue) {
  return makeSchema(schema.n, schema.t, schema.m, (b, input, selfSchema, path) => {
    ...
    return withCatch(b, input, (b, errorVar) => ({
      ...
      a: false
    }), b => parseWithTypeValidation(b, schema, input, path));
...
function definitionToSchema(definition) {
  ...
  if (Array.isArray(definition)) {
    let length = definition.length;
    let reversedItems = new Array(length);
    let isTransformed = false;
    for (let idx = 0, idx_finish = definition.length; idx < idx_finish; ++idx) {
    ...
    return makeSchema(name$2, {
      TAG: "tuple",
      items: definition
    }, empty, builder$1, maybeTypeFilter, isTransformed ? () => makeReverseSchema(name$2, {
        TAG: "tuple",
        items: reversedItems
      }, empty, builder$1, maybeTypeFilter) : toSelf);
  }
  let fieldNames = Object.keys(definition);
  let length$1 = fieldNames.length;
  let items = new Array(length$1);
  for (let idx$1 = 0; idx$1 < length$1; ++idx$1) {
    let location$1 = fieldNames[idx$1];
    let inlinedLocation$1 = fromString(location$1);
